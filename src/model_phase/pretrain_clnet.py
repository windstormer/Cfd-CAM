...
if __name__ == '__main__':
    ...
    parser.add_argument("--method_type",
                        ...
                        default="SimCLR",
                        help="Method of the Contrastive Learning")
    ...
    record_path = f"record/{args.method_type}"
    ...
    assert args.model_type in ["Res18", "Res50"], 'error model type given'

    model_name = f"{args.model_type}_{args.modality}_ep{args.epochs}_b{args.batch_size}"

    if args.suffix != None:
        model_name = model_name + f".{args.suffix}"
    ...
    train_dataset = TrainDataset(data, args.patch_size)
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=False, num_workers=16,
                                pin_memory=True, drop_last=True, sampler=train_sampler)
    val_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=False, num_workers=16,
                            pin_memory=True, drop_last=True, sampler=test_sampler)

    net = CLNet(args, train_loader, val_loader, full_log_path, record_path, model_name, gpuid)
