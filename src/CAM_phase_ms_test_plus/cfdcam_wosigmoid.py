...
class CfdCAM(object):
    ...
    def run_selected_case(self, loader):
        ...
        with torch.no_grad():
            for img_name, case_batch, seg_batch in test_bar:
                ...
                _, _, base_confidence, _ = self.step(torch.zeros_like(case_batch_1x))

                final_map_1x, final_seg_1x = self.CfdCAM_algo(input_image_1x, feat_maps, confidence, base_confidence, img_name)
                ...
                _, _, base_confidence, _ = self.step(torch.zeros_like(case_batch_2x))
                final_map_2x, final_seg_2x = self.CfdCAM_algo(input_image_2x, feat_maps, confidence, base_confidence, img_name)
    ...
    def CfdCAM_algo(self, input_image, feat_maps, confidence, base_confidence, img_name, print_feat_map=False, output_hist=False):
        ...
        # _, featured_conf, _, _ = self.step(featured_tensor.cuda())
        featured_tensor = []
        norm_feat_map = []
        for idx, feat_map in enumerate(feat_maps[0]):
            feat_map = F.interpolate(feat_map.unsqueeze(0).unsqueeze(1), size=img_size, mode='bilinear', align_corners=False)
        ...
        featured_conf = []
        bsize = 128
        for b in range(0, featured_tensor.shape[0], bsize):
            _, _, conf, _ = self.step(featured_tensor[b:b+bsize].cuda())
        ...
        cam_weight = featured_conf
        # cam_weight = torch.where(featured_conf < base_confidence[0], torch.zeros(1), featured_conf)


        if torch.sum(cam_weight)==0:
            cam_weight = cam_weight.numpy()
        else:
            # cam_weight = (cam_weight - cam_weight.min()) / (cam_weight.max() - cam_weight.min())
            cam_weight = (cam_weight/torch.sum(cam_weight)).numpy()
        # cam_weight = F.softmax(cam_weight, dim=0).numpy() 
        # feat_maps = (feat_maps - feat_maps.min()) / (feat_maps.max() - feat_maps.min())
        # final_map = torch.sum(feat_maps[0]*cam_weight[:, np.newaxis, np.newaxis], dim=0)
        # final_map = F.relu(final_map)
        
        # final_map = F.interpolate(final_map.unsqueeze(0).unsqueeze(1), size=img_size, mode='bilinear', align_corners=False)
        ...
        final_map = final_map * np.array(confidence[0])
