...
class CfdCAM(object):
    ...
    def run_selected_case(self, loader):
        ...
        with torch.no_grad():
            for img_name, case_batch, seg_batch in test_bar:
                ...
                _, base_confidence, _, _ = self.step(torch.zeros_like(case_batch))

                final_map, final_seg = self.CfdCAM_algo(input_image, feat_maps, confidence, base_confidence, img_name, print_feat_map=True, output_hist=True)
    ...
    def run_tumor_test(self, loader):
        ...
        with torch.no_grad():
            for img_name, case_batch, seg_batch in test_bar:
                ...
                final_map, final_seg = self.CfdCAM_algo(input_image, feat_maps, confidence, base_confidence, img_name)
    ...
    def CfdCAM_algo(self, input_image, feat_maps, confidence, base_confidence, img_name, print_feat_map=False, output_hist=False):
        ...
        if print_feat_map:
            ...
            for idx, masked_image in enumerate(featured_tensor):
                m = masked_image.permute(1,2,0).numpy()
                io.imsave(os.path.join(self.result_path, img_name, "feat_map", f"masked_{idx}.jpg"), img_as_ubyte(m), check_contrast=False)
        ...
        # cam_weight = F.softmax(cam_weight).numpy()  
